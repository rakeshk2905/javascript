...
const arrayTrans = arrayNum.map((currentValue) => {
  ...
  console.log(`=====WAP to get students name======`);
  const arrayOfName=arrayStudents.map((element)=>{
    return element.name;
  });
  console.log(arrayOfName);

  console.log(`=====WAP to get students id======`);
  const arrayOfid=arrayStudents.map((element)=>{
    return element.id;
  });
